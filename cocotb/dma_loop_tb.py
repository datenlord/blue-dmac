...
#  --------------        -------------         -----------
# Loop TB only runs for Simple Mode

DescBus, DescTransaction, DescSource, DescSink, DescMonitor = define_stream("Desc",
    signals=["start_addr", "byte_cnt", "is_write", "valid", "ready"]
)
class h2cHelper(object):
    def __init__(pcie_tb, dut, self):
        self._desc_addr_lo_0 = 0
        self._desc_addr_hi_0 = 1
        self._desc_len_0     = 2
        self._desc_cntl_0    = 3
        self._desc_addr_lo_1 = 4
        self._desc_addr_hi_1 = 5
        self._desc_len_1     = 6
        self._desc_cntl_1    = 7
        self._flag_addr_lo_0 = 8
        self._flag_addr_hi_0 = 9
        self._flag_addr_lo_1 = 10
        self._flag_addr_hi_1 = 11
        self._va_head_lo_0   = 12
        self._va_head_hi_0   = 13
        self._va_head_lo_1   = 12
        self._va_head_hi_1   = 13
        
        self._bar_offset = 0
        self._reg_block_size = 512
        self._phy_addr_table_size = 512
        
        self.tb = TB(dut)
        self.mem = self.tb.rc.mem_pool.alloc_region(1024*1024)
        
    async def init(self):
        await self.tb.gen_reset()
    
        await self.tb.rc.enumerate()
        dev = self.tb.rc.find_device(self.tb.dev.functions[0].pcie_id)
        
        await dev.enable_device()
        await dev.set_master()
        
    async def ioWrite32(self, address, value):
        raise NotImplementedError
    
    async def send_desc(self, channel, address, length, isWrite):
        if (channel == 0):
            addrLo, addrHi = self.splitDword2Word(address)
            await self.ioWrite32(self._desc_addr_lo_0, addrLo.to_bytes())
            await self.ioWrite32(self._desc_addr_hi_0, addrHi.to_bytes())
            await self.ioWrite32(self._desc_len_0, length.to_bytes())
            
        
    
    def set_bar_offset(self, offset):
        self._bar_offset = offset
        
    def splitDword2Word(dword):
        lo = dword & 0xFFFF
        hi = (dword >> 32) & 0xFFFF
        return (lo, hi)
